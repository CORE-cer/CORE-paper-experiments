...
# TESTS = ["Memory"]
TESTS = ['Time', "Memory"]
WIN_LENGTH = [10000, 20000, 30000, 40000]
# WIN_LENGTH = [100]
ITERATIONS = 3
TIMEOUTS = [30]
SYSTEMS = ['esper8', 'flink', 'core', 'sase']
# SYSTEMS = ['core']
# SYSTEMS = ['sase', 'esper8', 'core']
# SYSTEMS = ['flink']
TEST_NAME = ['taxi']
CONSUME = True
NUM_EVENT_DICT = {}
...
def create_core_query():
    ...
    with open(f'./results/{TEST_NAME[0]}/core/taxi.stream', 'w') as tf:
        tf.write('S:FILE:./taxistream/taxi.stream')
    for i in range(1, 13):
        with open(f'./taxiqueries/CORE/q{i}.txt') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME[0]}/core/core_declaration_taxi_q{i}_{win_length}.query', 'w') as tf:
                tf.write('FILE:./taxiqueries/CORE/descriptions/core.txt\n')
                tf.write(
                    f'FILE:./results/{TEST_NAME[0]}/core/core_taxi_q{i}_{win_length}.query\n')
            with open(f'./results/{TEST_NAME[0]}/core/core_taxi_q{i}_{win_length}.query', 'w') as tf:
...
def create_sase_query():
    ...
    for i in range(1, 13):
        with open(f'./taxiqueries/SASE/q{i}.query') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME[0]}/sase/sase_taxi_q{i}_{win_length}.query', 'w') as tf:
...
def create_esper_query():
    ...
    for i in range(1, 13):
        with open(f'./taxiqueries/ESPER/q{i}.query') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME[0]}/esper/esper_taxi_q{i}_{win_length}.query', 'w') as tf:
                tf.write(original.replace('TIMESTAMP', f'{win_length}'))
        ...
        for query in range(1, 13):
            for system in SYSTEMS:
                ...
                if test == 'Memory':
                    memorytest = True
                for win_length in WIN_LENGTH:
                    if win_length != 10000 and query < 11:
                        continue
                    for j in range(len(TIMEOUTS)):
                        ...
                        for i in range(ITERATIONS):
                            ...
                            try:
                                if system == 'core':
                                    print(
                                        f'Running core with query core_taxi_q{query}_{win_length}.query, stream taxi.stream. Memorytest: {memorytest}')
                                    t0 = time.time_ns()
                                    res = run_core(win_length, query, memorytest, timeout, max_events)
                                    total_time = time.time_ns() - t0
                                elif system == 'core1':
                                    print(
                                        f'Running core1 with query core_taxi_q{query}_{win_length}.query, stream taxi.stream. Memorytest: {memorytest}')
                                    t0 = time.time_ns()
                                    res = run_core1(win_length, query, memorytest, timeout, max_events)
                                    total_time = time.time_ns() - t0
                                elif system == 'sase':
                                    print(
                                        f'Running sase with query sase_taxi_q{query}_{win_length}.query, stream taxi.stream. Memorytest: {memorytest}')
                                    t0 = time.time_ns()
                                    res = run_sase(win_length, query, memorytest, timeout, max_events)
                                    total_time = time.time_ns() - t0
                                elif system == 'esper8':
                                    print(
                                        f'Running esper8 with query esper_taxi_q{query}_{win_length}.query, stream taxi.stream. Memorytest: {memorytest}')
                                    if not CONSUME:
                                        break
                                    t0 = time.time_ns()
                                    ...
                                    print(
                                        f'Running flink with query flink_taxi_q{query}_{win_length}.query, stream taxi.stream. Memorytest: {memorytest}')
                                    if not CONSUME:
                                        break
                                    t0 = time.time_ns()
                                    ...
                                    continue
                                with open(f'./results/{TEST_NAME[0]}/results/{system}_taxi_q{query}_{win_length}_{test}.query' + '_out.txt', 'ab') as tf:
                                    if not j and not i:
                                        if memorytest:
                                            tf.write(
                                    ...
                                    tf.write(res.stdout)
                                with open(f'./results/{TEST_NAME[0]}/results/{system}_taxi_q{query}_{win_length}_{test}.query' + '_err.txt', 'ab') as tf:
                                    tf.write(res.stderr)
                                print(
                                    f'successfully ran {system} query {system}_taxi_q{query}_{win_length}.query with stream taxi.stream.')
                                if not memorytest:
                                ...
                                with open(f'./results/{TEST_NAME[0]}/results/{system}_taxi_q{query}_{win_length}_{test}.query' + '_except.txt', 'a') as tf:
...
def run_core(win_length, query, memorytest, timeout, max_events):
    return subprocess.run(['C:/Users/AndresQuintana/.jdks/jbr-11.0.12/bin/java', '-Xmx50G',
                           ...
                           '-of',
                           '-q', f'./results/{TEST_NAME[0]}/core/core_declaration_taxi_q{query}_{win_length}.query',
                           '-s', f'./results/{TEST_NAME[0]}/core/taxi.stream',
                           '-m', f'{memorytest}',
                           '-t', f'{timeout}',
                           '-n', f'{max_events}',
                           '-i', '1000',
...
def run_sase(win_length, query, memorytest, timeout, max_events):
    return subprocess.run(['C:/Users/AndresQuintana/.jdks/jbr-11.0.12/bin/java', '-Xmx50G',
                           '-jar', './jars/sase.jar',
                           f'./results/{TEST_NAME[0]}/sase/sase_taxi_q{query}_{win_length}.query',
                           f'./taxistream/taxi.stream',
                           f'{CONSUME}', f'{memorytest}', f'{True}', f'{max_events}', f'{3}', f'{timeout}'],
                          timeout=timeout * 2, capture_output=True, check=True)


def run_esper8(win_length, query, memorytest, timeout, max_events):
    return subprocess.run(['C:/Users/AndresQuintana/.jdks/jbr-11.0.12/bin/java', '-Xmx50G',
                           '-jar', './jars/espertaxi.jar',
                           f'./results/{TEST_NAME[0]}/esper/esper_taxi_q{query}_{win_length}.query',
                           f'./taxistream/taxi.stream',
