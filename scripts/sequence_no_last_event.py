...
ITERATIONS = 3
TIMEOUTS = [30]
SYSTEMS = ['flink', 'sase', 'core', 'esper8']
# SYSTEMS = ['core', 'sase']
# SYSTEMS = ['esper']
...
def create_core_query():
    os.mkdir(f'./results/{TEST_NAMES}/core')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAMES}/core/core_declaration_{events_num}_{win_length}.query', 'w') as tf:
                tf.write(
                    f'FILE:./results/{TEST_NAMES}/core/core_metadata_{events_num}_{win_length}.meta\n')
                tf.write(
                    f'FILE:./results/{TEST_NAMES}/core/core_{events_num}_{win_length}.query\n')
            with open(f'./results/{TEST_NAMES}/core/core_metadata_{events_num}_{win_length}.meta', 'w') as tf:
                for event in events:
                    tf.write(f'DECLARE EVENT {event}(id long)\n')
                tf.write(f'DECLARE EVENT ZZ(id long)\n')
                tf.write(f'DECLARE STREAM S({", ".join(events)}, ZZ)')
            with open(f'./results/{TEST_NAMES}/core/core_{events_num}_{win_length}.query', 'w') as tf:
                tf.write('SELECT ALL *\n')
                tf.write('FROM S\n')
                tf.write('WHERE (')
        ...
        for noise_events in NOISE_EVENTS:
            with open(f'./results/{TEST_NAMES}/core/{events_num}_{noise_events}_{STREAM_LENGTH}.stream', 'w') as tf:
                tf.write(
...
def run_core(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(['java', '-Xmx50G',
                           ...
                           '-of',
                           '-q', f'./results/{TEST_NAMES}/core/core_declaration_{events_num}_{win_length}.query',
                           '-s', f'./results/{TEST_NAMES}/core/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
                           '-m', f'{memorytest}',
                           '-t', f'{timeout}',
                           '-n', f'{max_events}'],
