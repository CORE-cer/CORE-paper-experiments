...
# TESTS = ['Memory']
STREAM_EVENTS = [3]
STREAM_LENGTH = 75_000_000
WIN_LENGTH = [100]
NOISE_EVENTS = [6]
ITERATIONS = 3
TIMEOUTS = [30]
# SYSTEMS = ['flink', 'sase', 'esper8', 'coreall', 'corenext', 'coremax', 'corelast']
SYSTEMS = ['coreall', 'corenext', 'coremax', 'corelast']
# SYSTEMS = ['core']
# SYSTEMS = ['flink']
TEST_NAMES = ['seq_sel4']
CONSUME = True
...
def create_folder():
    os.mkdir(f'./results/{TEST_NAMES}')

def create_streams():
    print('Creating streams...')
    os.mkdir(f'./results/{TEST_NAMES}/streams')
    for events_num in STREAM_EVENTS:
        for noise_events in NOISE_EVENTS:
            ...
            with open(f'./results/{TEST_NAMES}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream', 'w') as tf:
...
def create_queries():
    ...
    for system in SYSTEMS:
        if system == 'coreall':
            ...
            create_core_query('MAX')
        elif system == 'corenext':
            create_core_query('NEXT')
        elif system == 'corelast':
            create_core_query('LAST')
        elif system == 'sase':
            create_sase_query()
        elif system == 'esper8':
            create_esper_query()
...
def create_core_query(selection='ALL'):
    os.mkdir(f'./results/{TEST_NAMES}/core{selection}')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAMES}/core{selection}/core_{selection}_declaration_{events_num}_{win_length}.query', 'w') as tf:
                tf.write(
                    f'FILE:./results/{TEST_NAMES}/core{selection}/core_{selection}_metadata_{events_num}_{win_length}.meta\n')
                tf.write(
                    f'FILE:./results/{TEST_NAMES}/core{selection}/core_{selection}_{events_num}_{win_length}.query\n')
            with open(f'./results/{TEST_NAMES}/core{selection}/core_{selection}_metadata_{events_num}_{win_length}.meta', 'w') as tf:
                for event in events:
                    tf.write(f'DECLARE EVENT {event}(id long)\n')
                tf.write(f'DECLARE EVENT ZZ(id long)\n')
                tf.write(f'DECLARE STREAM S({", ".join(events)}, ZZ)')
            with open(f'./results/{TEST_NAMES}/core{selection}/core_{selection}_{events_num}_{win_length}.query', 'w') as tf:
                tf.write(f'SELECT {selection} *\n')
                tf.write('FROM S\n')
                tf.write('WHERE (')
                tf.write(f'{"; ".join(events_query)}; ZZ')
                tf.write(')\n')
        ...
        for noise_events in NOISE_EVENTS:
            with open(f'./results/{TEST_NAMES}/core{selection}/{events_num}_{noise_events}_{STREAM_LENGTH}.stream', 'w') as tf:
                tf.write(
                    f'S:FILE:./results/{TEST_NAMES}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream')


def create_sase_query():
    os.mkdir(f'./results/{TEST_NAMES}/sase')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAMES}/sase/sase_{events_num}_{win_length}.query', 'w') as tf:
                ...
                for i in range(len(events_query)):
                    if i == 0:
                        ...
                        tf.write(f', {events_query[i] + " " + events_query[i].lower()}')
                tf.write(', ZZ zz)\n')
                tf.write('WHERE skip-till-next-match\n')
                tf.write(f'WITHIN {win_length - 1}')


def create_esper_query():
    os.mkdir(f'./results/{TEST_NAMES}/esper')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        ...
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAMES}/esper/esper_{events_num}_{win_length}.query', 'w') as tf:
                tf.write(
                    f'select {", ".join(events_lower)}, zz from MyEvent#length({win_length})\n')
                ...
                tf.write('\n')
                tf.write(f'pattern (')
                tf.write(f'{" s* ".join(events_query)} s* ZZ')
                tf.write(')\n')
                tf.write('define\n')
                last = events_query[-1]
...
def create_siddhi_query():
    os.mkdir(f'./results/{TEST_NAMES}/siddhi')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAMES}/siddhi/siddhi_{events_num}_{win_length}.query', 'w') as tf:
                tf.write('define stream InputStream (type string, idx int);\n')
                tf.write('define stream QueryStream (type string, idx int);\n')

                tf.write('from InputStream\n')
                tf.write('insert into QueryStream;\n')
                tf.write("@info(name = 'query1')\n")

                for i in range(len(events_query)):
                    if i == 0:
                        tf.write(f"from every e{i} = QueryStream[type == '{events_query[i]}']\n")
                    elif i == len(events_query) - 1:
                        tf.write(f"  -> e{i} = QueryStream[type == '{events_query[i]}' and idx < e0.idx + {win_length} ]\n")
                    else:
                        tf.write(f"  -> e{i} = QueryStream[type == '{events_query[i]}']\n")
                tf.write('select ')
                for i in range(len(events_query)):
                    if i == len(events_query) - 1:
                        tf.write(f"e{i}.idx as e{i}\n")
                    else:
                        tf.write(f"e{i}.idx as e{i}, ")
                tf.write(f'insert into OutputStream;')

def run_systems():
    print('Running systems...')
    if not os.path.exists(f'./results/{TEST_NAMES}/results'):
        os.mkdir(f'./results/{TEST_NAMES}/results')
    for test in TESTS:
                        ...
                        for j in range(len(TIMEOUTS)):
                            ...
                            for i in range(ITERATIONS):
                                ...
                                try:
                                    if system == 'coreall':
                                        print(
                                            f'Running coreall with query core_{events_num}_{win_length}.query, stream {events_num}_{noise_events}_{STREAM_LENGTH}.stream. Memorytest: {memorytest}')
                                        t0 = time.time_ns()
                                        res = run_core(
                                        ...
                                        print(
                                            f'Running coremax with query core_{events_num}_{win_length}.query, stream {events_num}_{noise_events}_{STREAM_LENGTH}.stream. Memorytest: {memorytest}')
                                        t0 = time.time_ns()
                                        res = run_core(
                                            events_num, win_length, noise_events, memorytest, timeout, max_events, 'MAX')
                                        total_time = time.time_ns() - t0
                                    elif system == 'corenext':
                                        print(
                                            f'Running corenext with query core_{events_num}_{win_length}.query, stream {events_num}_{noise_events}_{STREAM_LENGTH}.stream. Memorytest: {memorytest}')
                                        t0 = time.time_ns()
                                        res = run_core(
                                            events_num, win_length, noise_events, memorytest, timeout, max_events, 'NEXT')
                                        total_time = time.time_ns() - t0
                                    elif system == 'corelast':
                                        print(
                                            f'Running corelast with query core_{events_num}_{win_length}.query, stream {events_num}_{noise_events}_{STREAM_LENGTH}.stream. Memorytest: {memorytest}')
                                        t0 = time.time_ns()
                                        res = run_core(
                                            events_num, win_length, noise_events, memorytest, timeout, max_events, 'LAST')
                                        total_time = time.time_ns() - t0
                                    elif system == 'sase':
                                        print(
                                        ...
                                        sys.exit(1)
                                    with open(f'./results/{TEST_NAMES}/results/{system}_{events_num}_{win_length}_{noise_events}_{test}.query' + '_out.txt', 'ab') as tf:
                                        if not j and not i:
                                            if memorytest:
                                                tf.write(
                                        ...
                                        tf.write(res.stdout)
                                    with open(f'./results/{TEST_NAMES}/results/{system}_{events_num}_{win_length}_{noise_events}_{test}.query' + '_err.txt', 'ab') as tf:
                                        tf.write(res.stderr)
                                    print(
                                    ...
                                    with open(f'./results/{TEST_NAMES}/results/{system}_{events_num}_{win_length}_{noise_events}_{test}.query' + '_except.txt', 'a') as tf:
...
def run_core(events_num, win_length, noise_events, memorytest, timeout, max_events, selection='ALL'):
    return subprocess.run(['java', '-Xmx50G',
                           '-jar', './jars/core.main.jar',
                           '-of',
                           '-q', f'./results/{TEST_NAMES}/core{selection}/core_{selection}_declaration_{events_num}_{win_length}.query',
                           '-s', f'./results/{TEST_NAMES}/core{selection}/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
                           '-m', f'{memorytest}',
                           '-t', f'{timeout}',
                           '-n', f'{max_events}'],
...
def run_sase(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(['java', '-Xmx50G',
                           '-jar', './jars/sase.jar',
                           f'./results/{TEST_NAMES}/sase/sase_{events_num}_{win_length}.query',
                           f'./results/{TEST_NAMES}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
...
def run_esper(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(['java', '-Xmx50G',
                           '-jar', './jars/esper.jar',
                           f'./results/{TEST_NAMES}/esper/esper_{events_num}_{win_length}.query',
                           f'./results/{TEST_NAMES}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
...
def run_flink(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(["java", "-Dfile.encoding=windows-1252", "-Duser.country=US", "-Duser.language=en", "-Duser.variant", 
                            ...
                            "edu.puc.flink.Main",
                            f'./results/{TEST_NAMES}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
                            "seq", f"{events_num} {win_length}", 'true', f'{timeout}', f'{memorytest}', f'{max_events}'],
                          timeout=timeout * 10, capture_output=True, check=True)

def main():
    print(f'Starting test with TEST_NAMES {TEST_NAMES}')
    create_folder()
    create_streams()
    create_queries()
    run_systems()
