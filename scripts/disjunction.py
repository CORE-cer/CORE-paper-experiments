...
TIMEOUTS = [30]
# TIMEOUTS = [10, 15]
SYSTEMS = ['flink', 'core', 'esper8']
TIMESTAMP = ['test1']
CONSUME = True
NUM_EVENT_DICT = {}

def create_folder():
    os.mkdir(f'./results/{TIMESTAMP[0]}')

def create_streams():
    print('Creating streams...')
    os.mkdir(f'./results/{TIMESTAMP[0]}/streams')
    for events_num in STREAM_EVENTS:
        for noise_events in NOISE_EVENTS:
            ...
            with open(f'./results/{TIMESTAMP[0]}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream', 'w') as tf:
...
def create_core_query():
    os.mkdir(f'./results/{TIMESTAMP[0]}/core')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        for win_length in WIN_LENGTH:
            with open(f'./results/{TIMESTAMP[0]}/core/core_declaration_{events_num}_{win_length}.query', 'w') as tf:
                tf.write(
                    f'FILE:./results/{TIMESTAMP[0]}/core/core_metadata_{events_num}_{win_length}.meta\n')
                tf.write(
                    f'FILE:./results/{TIMESTAMP[0]}/core/core_{events_num}_{win_length}.query\n')
            with open(f'./results/{TIMESTAMP[0]}/core/core_metadata_{events_num}_{win_length}.meta', 'w') as tf:
                for event in events:
                    tf.write(f'DECLARE EVENT {event}(id long)\n')
                tf.write(f'DECLARE STREAM S({", ".join(events)})')
            with open(f'./results/{TIMESTAMP[0]}/core/core_{events_num}_{win_length}.query', 'w') as tf:
                tf.write('SELECT ALL *\n')
                tf.write('FROM S\n')
                tf.write('WHERE (')
        ...
        for noise_events in NOISE_EVENTS:
            with open(f'./results/{TIMESTAMP[0]}/core/{events_num}_{noise_events}_{STREAM_LENGTH}.stream', 'w') as tf:
                tf.write(
                    f'S:FILE:./results/{TIMESTAMP[0]}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream')


def create_esper_query():
    os.mkdir(f'./results/{TIMESTAMP[0]}/esper')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        ...
        for win_length in WIN_LENGTH:
            with open(f'./results/{TIMESTAMP[0]}/esper/esper_{events_num}_{win_length}.query', 'w') as tf:
                tf.write(
...
def run_systems():
    ...
    if not os.path.exists(f'./results/{TIMESTAMP[0]}/results'):
        os.mkdir(f'./results/{TIMESTAMP[0]}/results')
    for events_num in STREAM_EVENTS:
                            ...
                            for i in range(ITERATIONS):
                                ...
                                try:
                                    if system == 'core':
                                        ...
                                        sys.exit(1)
                                    with open(f'./results/{TIMESTAMP[0]}/results/{system}_{events_num}_{win_length}_{noise_events}_{test}.query' + '_out.txt', 'ab') as tf:
                                        if not j and not i:
                                            if memorytest:
                                                tf.write(
                                        ...
                                        tf.write(res.stdout)
                                    with open(f'./results/{TIMESTAMP[0]}/results/{system}_{events_num}_{win_length}_{noise_events}_{test}.query' + '_err.txt', 'ab') as tf:
                                        tf.write(res.stderr)
                                    print(
                                    ...
                                    with open(f'./results/{TIMESTAMP[0]}/results/{system}_{events_num}_{win_length}_{noise_events}_{test}.query' + '_except.txt', 'a') as tf:
...
def run_core(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(['java', '-Xmx50G',
                           ...
                           '-of',
                           '-q', f'./results/{TIMESTAMP[0]}/core/core_declaration_{events_num}_{win_length}.query',
                           '-s', f'./results/{TIMESTAMP[0]}/core/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
                           '-m', f'{memorytest}',
                           '-t', f'{timeout}',
                           '-n', f'{max_events}'],
...
def run_sase(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(['java', '-Xmx50G',
                           '-jar', './jars/sase.jar',
                           f'./results/{TIMESTAMP[0]}/sase/sase_{events_num}_{win_length}.query',
                           f'./results/{TIMESTAMP[0]}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
...
def run_esper(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(['java', '-Xmx50G',
                           '-jar', './jars/esper.jar',
                           f'./results/{TIMESTAMP[0]}/esper/esper_{events_num}_{win_length}.query',
                           f'./results/{TIMESTAMP[0]}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
...
def run_flink(events_num, win_length, noise_events, memorytest, timeout, max_events):
    return subprocess.run(["java", "-Dfile.encoding=windows-1252", "-Duser.country=US", "-Duser.language=en", "-Duser.variant", 
                            ...
                            "edu.puc.flink.Main",
                            f'./results/{TIMESTAMP[0]}/streams/{events_num}_{noise_events}_{STREAM_LENGTH}.stream',
                            "disj", f"{events_num} {win_length}", 'false', f'{timeout}', f'{memorytest}', f'{max_events}'],
                          timeout=timeout * 10, capture_output=True, check=True)

def main():
    if len(sys.argv) < 2:
        print(f'Starting test with timestamp[0] {TIMESTAMP[0]}')
        create_folder()
        create_streams()
        create_queries()
    else:
        TIMESTAMP[0] = int(sys.argv[1])
    run_systems()
