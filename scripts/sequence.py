...
TIMEOUTS = [30]
# SYSTEMS = ['siddhi']
SYSTEMS = ['flink', 'core', 'sase', 'esper8']
TEST_NAME = 'seq'
CONSUME = True
NUM_EVENT_DICT = {}
    ...
    for events_num in STREAM_EVENTS:
        ...
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME}/esper/esper_{events_num}_{win_length}.query', 'w') as tf:
                ...
                tf.write(f'pattern (')
                tf.write(f'{" s* ".join(events_query)}')
                tf.write(')\n')
                tf.write('define\n')
                last = events_query[-1]
...
def create_siddhi_query():
    os.mkdir(f'./results/{TEST_NAME}/siddhi')
    for events_num in STREAM_EVENTS:
        noise_events = max(NOISE_EVENTS)
        events = get_events(events_num + noise_events)
        events_query = [events[i] for i in range(events_num)]
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME}/siddhi/siddhi_{events_num}_{win_length}.query', 'w') as tf:
                ...
                for i in range(len(events_query)):
                    if i == 0:
                        ...
                        tf.write(f"  -> every e{i} = QueryStream[type == '{events_query[i]}']\n")
                tf.write('select ')
                for i in range(len(events_query)):
