...
ITERATIONS = 3
TIMEOUTS = [30]
SYSTEMS = ['flink', 'esper8', 'core', 'sase']
# SYSTEMS = ['esper8', 'core', 'sase', 'flink']
# SYSTEMS = ['flink']
TEST_NAME = ['new_stocks']
CONSUME = True
NUM_EVENT_DICT = {}
...
def create_core_query():
    ...
    with open(f'./results/{TEST_NAME[0]}/core/stocks.stream', 'w') as tf:
        tf.write('S:FILE:./stockstream/stocks.stream')
    for i in range(1, 13):
        with open(f'./stockqueries/CORE/q{i}.txt') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME[0]}/core/core_declaration_stocks_q{i}_{win_length}.query', 'w') as tf:
                tf.write('FILE:./stockqueries/CORE/descriptions/core.txt\n')
                tf.write(
                    f'FILE:./results/{TEST_NAME[0]}/core/core_stocks_q{i}_{win_length}.query\n')
            with open(f'./results/{TEST_NAME[0]}/core/core_stocks_q{i}_{win_length}.query', 'w') as tf:
...
def create_sase_query():
    ...
    for i in range(1, 13):
        with open(f'./stockqueries/SASE/q{i}.query') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME[0]}/sase/sase_stocks_q{i}_{win_length}.query', 'w') as tf:
...
def create_esper_query():
    ...
    for i in range(1, 13):
        with open(f'./stockqueries/ESPER/q{i}.query') as tf:
            original = tf.read()
        for win_length in WIN_LENGTH:
            with open(f'./results/{TEST_NAME[0]}/esper/esper_stocks_q{i}_{win_length}.query', 'w') as tf:
                tf.write(original.replace('TIMESTAMP', f'{win_length}'))
                        ...
                        for i in range(ITERATIONS):
                            ...
                            try:
                                if system == 'core':
                                    ...
                                    continue
                                with open(f'./results/{TEST_NAME[0]}/results/{system}_stocks_q{query}_{win_length}_{test}.query' + '_out.txt', 'ab') as tf:
                                    if not j and not i:
                                        if memorytest:
                                            tf.write(
                                    ...
                                    tf.write(res.stdout)
                                with open(f'./results/{TEST_NAME[0]}/results/{system}_stocks_q{query}_{win_length}_{test}.query' + '_err.txt', 'ab') as tf:
                                    tf.write(res.stderr)
                                print(
                                ...
                                with open(f'./results/{TEST_NAME[0]}/results/{system}_stocks_q{query}_{win_length}_{test}.query' + '_except.txt', 'a') as tf:
...
def run_core(win_length, query, memorytest, timeout, max_events):
    return subprocess.run(['C:/Users/AndresQuintana/.jdks/jbr-11.0.12/bin/java', '-Xmx50G',
                           ...
                           '-of',
                           '-q', f'./results/{TEST_NAME[0]}/core/core_declaration_stocks_q{query}_{win_length}.query',
                           '-s', f'./results/{TEST_NAME[0]}/core/stocks.stream',
                           '-m', f'{memorytest}',
                           '-t', f'{timeout}',
                           '-n', f'{max_events}',
                           '-i', '1000',
...
def run_sase(win_length, query, memorytest, timeout, max_events):
    return subprocess.run(['C:/Users/AndresQuintana/.jdks/jbr-11.0.12/bin/java', '-Xmx50G',
                           '-jar', './jars/sase.jar',
                           f'./results/{TEST_NAME[0]}/sase/sase_stocks_q{query}_{win_length}.query',
                           f'./stockstream/stocks.stream',
                           f'{CONSUME}', f'{memorytest}', f'{True}', f'{max_events}', f'{1}', f'{timeout}'],
                          timeout=timeout * 2, capture_output=True, check=True)
...
def run_esper8(win_length, query, memorytest, timeout, max_events):
    return subprocess.run(['C:/Users/AndresQuintana/.jdks/jbr-11.0.12/bin/java', '-Xmx50G',
                           ...
                           f'./results/{TEST_NAME[0]}/esper/esper_stocks_q{query}_{win_length}.query',
                           f'./stockstream/stocks.stream',
