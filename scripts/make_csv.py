...
CSV_HEADER = 'System,Number of Events,Window Length,Noise_events,Timeout,Events,EnumTime,Matches,Throughput,MAXTotal,AVGTotal,MAXUsed,AVGUsed\n'
ITERATIONS = 3
TEST_NAMES = ['kleene2']#, 'seq_sel', 'seq_no_last_event']
...
def collect_data(timestamp):
    ...
    for f in os.listdir(f'./results/{timestamp}/results'):
        if f.endswith('err.txt'):
            continue
        f_data = f.split('.')[0]
        system, events, window, noise, test = f_data.split('_')
        if test == 'Time':
        ...
        if f.endswith('except.txt'):
            ...
            data_dict[(system, events, window, noise)] = [f'{timeout},-1,-1,-1,-1',
                                                            f'{timeout},-1,-1,-1,-1',
                                                            f'{timeout},-1,-1,-1,-1']
            continue
        ...
        if test == 'Time':
            ...
            for d in data:
                temp = d.split(',')
                temp.append(str(float(temp[1])/float(temp[0])))
                newdata.append(','.join(temp))
            data = newdata
        data_dict[(system, events, window, noise)] = data
    merged_data_dict = {}
    ...
    with open(f'./csv/all_data_{timestamp}.csv', 'w') as tf:
        ...
        for k, v in merged_data_dict.items():
            system, events, window, noise = k
            for d in v:
                tf.write(f'{system},{events},{int(window)},{noise},{d}\n')
        ...
        for k, v in merged_data_dict.items():
            ...
            for d in range(len(v)):
                ...
                if not (d + 1) % ITERATIONS: 
                    ...
                    tf.write(f'{system},{events},{int(window)},{noise},{",".join(sum_data)}\n')
                    data_list = []
